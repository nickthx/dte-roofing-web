...
| 260922-h4n | Align homepage WebPage JSON-LD `name` with the `<title>` tag. `src/pages/Home.tsx:32` SchemaMarkup `pageTitle` changed from "DTE Roofing - Columbus's Highest-Rated Roofing Contractor" to "Roof Repair and Replacement in Columbus, OH | DTE Roofing" (matches the SEO `title` prop and og:title). One-line diff; `SchemaMarkup.tsx` untouched (`generateWebPageSchema()` maps `pageTitle` → `name`, breadcrumb generator returns null for home). `npm run build` clean (41 routes); `dist/index.html` has the new name exactly once and zero hits for "Highest-Rated". Found via live `/searchfit-seo:seo-check` of dteroofingllc.com (score 94/100, this was the only real fix). DEPLOYED + VERIFIED live 2026-09-22: pushed `1b77b74..d6a42e5`, deployment `dpl_Afv5GNfahPoMyWCLUAKCkNM1cinw` READY on production; live homepage has the new WebPage `name` exactly once and zero hits for "Highest-Rated". | 2026-09-22 | 5d46b54, d6a42e5 | [260922-h4n-align-homepage-webpage-schema-name-with-](./quick/260922-h4n-align-homepage-webpage-schema-name-with-/) |
